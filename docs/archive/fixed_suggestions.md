...
## Bug 1 — `useValue`: missing dependency array in `useEffect` [FIXED]
...
## Bug 2 — `useNano`: same missing dependency array [FIXED]
...
## Bug 3 — `derive.set()` notifies subscribers twice [FIXED]
...
## Bug 4 — `splitAtom`: atom list goes stale when array length changes [FIXED]
...
## Bug 5 — `batch`: type mismatch and non-composable [FIXED]
...
## Bug 6 — `unsub` error message says "Couldn't add" [FIXED]
...
## Bug 7 — `allowFnValue` guard silently swallows errors [FIXED]
